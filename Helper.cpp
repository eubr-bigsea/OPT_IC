...
void help::create_vector(const std::string& file_name,
                         std::vector<std::vector<std::string>>& v) {
  // Read the input file lines. Each line is stored as a vector of string and
  // finally stored in a vector of rows
  std::ifstream in(file_name);
  if (in) {
    std::string line;
    while (getline(in, line)) {  // read the current row
      std::stringstream sep(line);
      v.push_back(
          std::vector<std::string>());  // insert in v an empty row to be filled
      std::string string_name;
      while (getline(sep, string_name, ',')) {  // read words comma separated
