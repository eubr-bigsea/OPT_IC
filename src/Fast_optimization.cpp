...
void Fast_optimization::set_alpha_beta(const unsigned n1, const unsigned n2,
  ...
  if (n1 > n2) {
    alpha = static_cast<double>(r2 - r1) * n1 * n2 / (n1 - n2);
    beta = static_cast<double>(r1) - alpha / n1;
  } else {
    alpha = static_cast<double>(r1 - r2) * n1 * n2 / (n2 - n1);
    beta = static_cast<double>(r1) - alpha / n1;
  }
}
...
unsigned Fast_optimization::optimize(const bool no_ml) {
  std::cout << "---------------------------------------------------------------"
            << std::endl;
  std::cout << "\nStarting fast optimization - deadline: " << deadline
            << " ms\n"
            << std::endl;

  // if the evaluated time is less then the deadline with just one virtual
  // machine
  // the result is 1
  tmin = evaluate_time(numin);
  if (tmin < deadline) {
    std::cout << "Ncores = " << numin << " -> time = " << tmin << " ms"
              << std::endl;
    return numin;
  }

  // if the evaluated time with max_n_cores if greater then the deadline we
  // can't do
  // any better, the deadline is too restrictive
  tmax = evaluate_time(numax);
  if (tmax > deadline) {
    std::cout << "Ncores = " << numax << " -> time = " << tmax << " ms"
              << std::endl;
    std::cerr << "The deadline is too restrictive" << std::endl;
    return 0;
  }

  // we evaluate the nu computed with the initial alpha and beta and then we
  // assign
  // it nu numin or numax
  unsigned nu = (no_ml == true ? get_nu(alphain, betain)
                               : mlm.initial_core_numbers(ic, deadline));

  opt_common::TimeInstant t = evaluate_time(nu);
  std::cout << "Ncores (alpha & beta) = " << nu << " -> time = " << t << " ms"
            << std::endl;
  ...
  while (numax - numin != 1) {
    ...
    if (is_oscillating(nu)) {
      nu = std::ceil(static_cast<double>(numin + numax) / 2.);
    }
...
// if t is greater then the deadline the new nu becomes numin and t becomes
// timin;
// otherwise nu becomes numax and t becomes tmax
void Fast_optimization::assigne_nu_time(const unsigned nu,
                                        const opt_common::TimeInstant& t) {
  if (t > deadline) {
    numin = nu;
    tmin = t;
  } else {
