...
unsigned Fast_bisection_optimization::optimize(const bool no_ml) {
  std::cout << "---------------------------------------------------------------"
            << std::endl;
  std::cout << "\nStarting fast optimization with bisection - deadline: "
            << deadline << " ms\n"
            << std::endl;
  ...
  // while nu isn't oscillating its time is evaluated and assigned to numax or
  // numin,
  ...
  // if we exit from the while loop we go on with the bisection until the final
  // result is found
  bisection();
...
void Fast_bisection_optimization::bisection() {
  unsigned nu;
  opt_common::TimeInstant t;

  std::cout << "Starting bisection" << std::endl;

  // we don't take into account as an iteration the one that has caused the
  // oscillation
  it--;

  while (numax - numin != 1) {
    nu = std::ceil(static_cast<double>(numax + numin) / 2.);
    t = evaluate_time(nu);
    it++;
